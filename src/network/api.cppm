...
public:
  ...
  void uploadPrekeys(const std::string &accessToken,
                     const std::vector<std::string> &opkPubsB64) {
    m_http.post("/keys/prekeys", {{"one_time_prekeys", opkPubsB64}},
                accessToken);
  }
  ...
  void
  removeGroupMember(const std::string &accessToken, int32_t groupId,
                    int32_t userId,
    ...
    m_http.del("/groups/" + std::to_string(groupId) + "/members/" +
  ...
  void acknowledgeGroupReceipt(const std::string &accessToken, int32_t groupId,
                               int32_t messageId) {
    m_http.postEmpty("/groups/" + std::to_string(groupId) + "/messages/" +
                         std::to_string(messageId) + "/receipt",
                     accessToken);
  }

  void revokeGroupMessage(const std::string &accessToken, int32_t groupId,
                          int32_t messageId) {
    m_http.del("/groups/" + std::to_string(groupId) + "/messages/" +
