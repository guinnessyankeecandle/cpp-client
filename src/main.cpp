...
static Element qrElement(const std::string& uri) {
    ...
    std::string line;
    while (std::getline(ss, line)) rows.push_back(text(line));
    return vbox(std::move(rows));
}

// Forward declarations
static void startPolling(AppState& state, ScreenInteractive& scr, ApiClient& api);
...
Component makeWelcomeScreen(AppState& state, ScreenInteractive& scr) {
    auto btnRegister = Button(" Register ", [&]{ state.screen = AppScreen::Register; scr.PostEvent(Event::Custom); });
    auto btnLogin    = Button(" Login ",    [&]{ state.screen = AppScreen::Login;    scr.PostEvent(Event::Custom); });

    return Renderer(Container::Horizontal({btnRegister, btnLogin}), [&, btnRegister, btnLogin] {
        return vbox({
            filler(),
            hbox({ filler(),
                vbox({
                    ...
                    separator(),
                    hbox({ filler(), btnRegister->Render(), text("  "), btnLogin->Render(), filler() }),
...
Component makeRegisterScreen(AppState& state, ScreenInteractive& scr, ApiClient& api) {
    auto rUser = Input(&state.regUsername, "username");
    auto rPass = Input(&state.regPassword, "password", InputOption{.password = true});
    ...
    auto btnSubmit = Button(" Register ", [&] {
        if (state.regUsername.empty() || state.regPassword.empty()) {
            state.regStatus = "Username and password required."; return;
        }
        try {
    ...
    auto btnVerifyTotp = Button(" Verify ", [&] {
        ...
        try {
            ...
            if (!srp.verifyServerProof(verify["server_proof"].get<std::string>())) {
                state.regTotpStatus = "Server proof invalid."; return;
            }
            auto tokens = api.verify2FA(verify["pre_auth_token"].get<std::string>(), state.regTotpCode);
            ...
            api.publishKeyBundle(state.accessToken,
                ...
                base64Encode(state.keyBundle.spkSig),
                opkPubs,
    ...
    auto btnBack = Button(" Back ", [&] {
        ...
        state.regStatus.clear(); state.regTotpUri.clear(); state.regShowTotp = false;
        scr.PostEvent(Event::Custom);
    });

    return Renderer(Container::Vertical({rUser, rPass, tCode, btnSubmit, btnVerifyTotp, btnBack}),
        ...
        if (!state.regShowTotp) {
            ...
            body.push_back(hbox({filler(), btnSubmit->Render(), text("  "), btnBack->Render(), filler()}));
        }
        if (!state.regStatus.empty())
            body.push_back(paragraph(" " + state.regStatus)
                | color(state.regStatus.rfind("Error", 0) == 0 ? Color::Red : Color::Green));
        if (state.regShowTotp) {
            body.push_back(separator());
            if (!state.regTotpUri.empty()) body.push_back(qrElement(state.regTotpUri) | center);
            body.push_back(separator());
            body.push_back(hbox({text(" TOTP Code : "), tCode->Render() | flex}));
            body.push_back(separator());
            body.push_back(hbox({filler(), btnVerifyTotp->Render(), text("  "), btnBack->Render(), filler()}));
            if (!state.regTotpStatus.empty())
                body.push_back(text(" " + state.regTotpStatus) | color(Color::Red));
        }
        return vbox({filler(),
            hbox({filler(), vbox(std::move(body)) | border | size(WIDTH, GREATER_THAN, 52), filler()}),
        filler()});
    });
}

Component makeLoginScreen(AppState& state, ScreenInteractive& scr, ApiClient& api) {
    auto lUser = Input(&state.loginUsername, "username");
    auto lPass = Input(&state.loginPassword, "password", InputOption{.password = true});
    ...
    auto btnLogin = Button(" Login ", [&] {
        if (state.loginUsername.empty() || state.loginPassword.empty()) {
            state.loginStatus = "Username and password required."; return;
        }
        try {
            ...
            auto verify = api.srpVerify(init["session_id"],
                            srp.computeProof(init["srp_salt"], init["server_public"]));
            if (!srp.verifyServerProof(verify["server_proof"].get<std::string>())) {
                state.loginStatus = "ERROR: Server proof invalid -- possible MITM!"; return;
            }
    ...
    auto btnVerify = Button(" Verify ", [&] {
        if (state.loginTotpCode.empty()) { state.loginTotpStatus = "Enter the 6-digit code."; return; }
        try {
    ...
    auto btnBack = Button(" Back ", [&] {
        state.screen = AppScreen::Welcome;
        state.loginStatus.clear(); state.loginShowTotp = false;
        scr.PostEvent(Event::Custom);
    });

    return Renderer(Container::Vertical({lUser, lPass, tCode, btnLogin, btnVerify, btnBack}),
        ...
        if (!state.loginShowTotp) {
            body.push_back(separator());
            body.push_back(hbox({filler(), btnLogin->Render(), text("  "), btnBack->Render(), filler()}));
            ...
            body.push_back(hbox({text(" TOTP Code : "), tCode->Render() | flex}));
            body.push_back(separator());
            body.push_back(hbox({filler(), btnVerify->Render(), text("  "), btnBack->Render(), filler()}));
...
static void stopPolling(AppState& state) {
    ...
    if (state.pollThread.joinable()) state.pollThread.join();
}
...
Component makeMainScreen(AppState& state, ScreenInteractive& scr, ApiClient& api) {
    static bool pollingStarted = false;
    if (!pollingStarted) { pollingStarted = true; startPolling(state, scr, api); }

    auto composeInput = Input(&state.composeText, "Type a message...");

    auto btnSend = Button(" Send ", [&] {
        if (state.composeText.empty()) return;
        try {
    ...
    const auto rightPanel = Renderer(Container::Vertical({composeInput, btnSend}),
        ...
        if (msgs.empty()) msgs.push_back(text(" No messages yet ") | dim | center);
        return vbox({
    ...
    auto overlayComp = Renderer(
        ...
        if (!state.showIdentityOverlay) return text("");
        Elements body = {
            text(" " + state.overlayTargetName + "'s Identity Key ") | bold | center,
            separator(),
            ...
            text(state.overlayVerified
                 ...
                 : " Not yet verified -- compare with contact directly ") | dim | center,
            separator(),
            hbox({filler(), btnCloseOverlay->Render(), text("  "), btnMarkVerified->Render(), filler()}),
        };
        return vbox({filler(),
            hbox({filler(), vbox(std::move(body)) | border | size(WIDTH, GREATER_THAN, 60), filler()}),
        filler()});
    });

    int tabIdx = 0;
    return CatchEvent(
        Renderer(Container::Tab({split, overlayComp}, &tabIdx), [&, split, overlayComp] {
            tabIdx = state.showIdentityOverlay ? 1 : 0;
            auto base = vbox({
                hbox({text(" SecureMsg ") | bold, filler(),
                      text(" [i] identity  [q] quit ") | dim}) | bgcolor(Color::Blue),
                split->Render() | flex,
            });
    ...
    const auto root = CatchEvent(
        Renderer(Container::Tab({welcome, reg, login, mainScr}, &screenIdx),
            [&, welcome, reg, login, mainScr] {
                ...
                switch (state.screen) {
                    case AppScreen::Welcome:  return welcome->Render();
                    case AppScreen::Register: return reg->Render();
                    case AppScreen::Login:    return login->Render();
                    case AppScreen::Main:     return mainScr->Render();
                }
                return text("");
            }),
