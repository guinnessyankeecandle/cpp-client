...
export std::vector<uint8_t> hkdf(const std::vector<uint8_t>& ikm,
    ...
    OSSL_PARAM params[5];
    int pi = 0;
    params[pi++] = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_DIGEST,
    ...
    if (!salt.empty())
        params[pi++] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT,
            const_cast<uint8_t*>(salt.data()), salt.size());
    params[pi++] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_INFO,
        const_cast<char*>(info.data()), info.size());
    params[pi]   = OSSL_PARAM_END;

    std::vector<uint8_t> out(outLen);
    sslAssert(EVP_KDF_derive(ctx.get(), out.data(), outLen, params), "HKDF derive");
    return out;
}
...
export std::vector<uint8_t> pbkdf2(const std::string&          password,
                                    ...
                                    std::size_t                 outLen) {
    std::vector<uint8_t> out(outLen);
    sslAssert(PKCS5_PBKDF2_HMAC(password.data(),
