...
static RemoteKeyBundle makeBundle(
        const X25519KeyPair&  receiverIk,
        Ed25519KeyPair&       signingIk,
        X25519KeyPair&        spk,
        MlKemKeyPair&         pqKp,
...
TEST_CASE("PQXDH sender-receiver roundtrip produces identical SK", "[pqxdh]") {
    ...
    auto receiverSk = pqxdhReceive(receiverIk, receiverSpk,
                                    std::nullopt, receiverPq, hdr);
...
TEST_CASE("PQXDH with OPK produces identical SK", "[pqxdh]") {
    ...
    auto bundle = makeBundle(receiverIk, receiverEd, receiverSpk,
                              receiverPq, receiverOpk);
    auto senderResult = pqxdhSend(senderIk, bundle);
