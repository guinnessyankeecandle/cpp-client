...
export Ed25519KeyPair ed25519Generate() {
    ...
    if (!ctx) throw std::runtime_error("EVP_PKEY_CTX_new_id Ed25519 failed");
...
export std::vector<uint8_t> ed25519Sign(const std::vector<uint8_t>& privKey,
    ...
    if (!pkey) throw std::runtime_error("Ed25519 new_raw_private_key failed");

    auto ctx = MdCtxPtr(EVP_MD_CTX_new());
    if (!ctx) throw std::runtime_error("EVP_MD_CTX_new failed");
    sslAssert(EVP_DigestSignInit(ctx.get(), nullptr, nullptr, nullptr, pkey.get()),
              "Ed25519 DigestSignInit");

    std::size_t sigLen = ED25519_SIG_BYTES;
    std::vector<uint8_t> sig(sigLen);
    sslAssert(EVP_DigestSign(ctx.get(), sig.data(), &sigLen,
                              message.data(), message.size()), "Ed25519 DigestSign");
    sig.resize(sigLen);
    return sig;
}
...
export bool ed25519Verify(const std::vector<uint8_t>& pubKey,
    ...
    if (!pkey) throw std::runtime_error("Ed25519 new_raw_public_key failed");

    auto ctx = MdCtxPtr(EVP_MD_CTX_new());
    if (!ctx) throw std::runtime_error("EVP_MD_CTX_new failed");
    sslAssert(EVP_DigestVerifyInit(ctx.get(), nullptr, nullptr, nullptr, pkey.get()),
