...
public:
  ...
  SrpProof computeProof(const std::string &username,
    ...
    // M1 = SHA256(SHA256(N) XOR SHA256(g) || SHA256(username) || salt || A || B
    // || K) Matches pysrp's non-RFC5054 M1 formula used by the server.
  ...
  // M2 = SHA256(A || M1 || K) — verifies the server also holds the correct
  // verifier.
  bool verifyServerProof(const std::string &serverProofHex) const {
    ...
    const auto serverM2 =
        Botan::BigInt::from_string("0x" + serverProofHex).serialize(32);
