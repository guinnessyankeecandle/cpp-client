...
export std::string srpComputeVerifier(const std::string& username,
    ...
    auto N = BnPtr(Nraw);
    auto g = BnPtr(BN_new()); BN_set_word(g.get(), SRP_G);

    std::string credentials = username + ":" + password;
    auto innerHash = sha256Multi({{reinterpret_cast<const uint8_t*>(credentials.data()),
                                   credentials.size()}});
    ...
    char* saltHex = BN_bn2hex(BN_bin2bn(saltPadded.data(),
        static_cast<int>(saltPadded.size()), nullptr));
...
public:
    ...
    std::string computeProof(const std::string& srpSaltHex,
        ...
        std::vector<uint8_t> nBytes(512); BN_bn2binpad(N.get(), nBytes.data(), 512);
        uint8_t gByte = SRP_G;
        ...
        std::vector<uint8_t> saltBytes(static_cast<std::size_t>(BN_num_bytes(salt.get())));
        BN_bn2bin(salt.get(), saltBytes.data());

        std::vector<uint8_t> BPad(512); BN_bn2binpad(B.get(), BPad.data(), 512);
        auto uBytes = sha256Multi({{m_A_bytes.data(), m_A_bytes.size()},
                                   {BPad.data(), BPad.size()}});
        auto u = BnPtr(BN_new());
        BN_bin2bn(uBytes.data(), static_cast<int>(uBytes.size()), u.get());

        std::string cred = m_username + ":" + m_password;
        auto inner  = sha256Multi({{reinterpret_cast<const uint8_t*>(cred.data()),
                                    cred.size()}});
        auto xBytes = sha256Multi({{saltBytes.data(), saltBytes.size()},
        ...
        std::vector<uint8_t> SBytes(512); BN_bn2binpad(S.get(), SBytes.data(), 512);
        ...
        for (int i = 0; i < 32; ++i) xorNG[i] = hashN[i] ^ hashG[i];
        auto hashUser = sha256Multi({{reinterpret_cast<const uint8_t*>(m_username.data()),
                                      m_username.size()}});
        ...
        BIGNUM* m1Bn = BN_bin2bn(m_M1.data(), static_cast<int>(m_M1.size()), nullptr);
    ...
    ~SrpSession() {
        if (!m_a_bytes.empty()) OPENSSL_cleanse(m_a_bytes.data(), m_a_bytes.size());
        if (!m_K.empty())       OPENSSL_cleanse(m_K.data(), m_K.size());
        if (!m_M1.empty())      OPENSSL_cleanse(m_M1.data(), m_M1.size());
    }
