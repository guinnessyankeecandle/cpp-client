...
static std::pair<std::vector<uint8_t>, std::vector<uint8_t>>
kdfRk(const std::vector<uint8_t>& rk,
      const std::vector<uint8_t>& dhOut) {
...
private:
    ...
    uint32_t                                             m_Nr{0};
    uint32_t                                             m_PN{0};
    std::map<std::pair<std::vector<uint8_t>, uint32_t>,
             std::vector<uint8_t>>                       m_MKSKIPPED;

    void skipMessageKeys(uint32_t until) {
        if (m_Nr > until) return;
    ...
    static std::vector<uint8_t> serialiseHeader(const RatchetHeader& h) {
        ...
        for (int i = 3; i >= 0; --i) out.push_back((h.pn >> (i*8)) & 0xFF);
        for (int i = 3; i >= 0; --i) out.push_back((h.n  >> (i*8)) & 0xFF);
        return out;
    }
    ...
    RatchetHeader decryptHeader(const std::vector<uint8_t>& hdrCt) const {
        ...
        if (bytes.size() < 40) throw std::runtime_error("Header too short");
        RatchetHeader h;
