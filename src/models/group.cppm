...
export class Group {
public:
    Group(const int32_t id, std::string name,
          std::vector<int32_t> members, const int32_t epoch)
        : group_id(id)
    ...
    [[nodiscard]] const std::vector<int32_t>& getMembers() const { return group_members; }
    ...
    void removeMember(const int32_t userId) {
        std::erase(group_members, userId);
    }
