...
struct CurlDeleter { void operator()(CURL* h) const { curl_easy_cleanup(h); } };
using CurlPtr = std::unique_ptr<CURL, CurlDeleter>;

struct CurlSlistDeleter { void operator()(curl_slist* s) const { curl_slist_free_all(s); } };
...
public:
    ...
    nlohmann::json del(const std::string& path,
        ...
        return request("DELETE", path,
                        body.is_null() ? "" : body.dump(), accessToken);
    }
...
private:
    ...
    nlohmann::json request(const std::string& method,
        ...
        if (responseCode == 429) {
            if (m_retryAfterCb) m_retryAfterCb(1);
        ...
        if (responseBody.empty()) return nullptr;
