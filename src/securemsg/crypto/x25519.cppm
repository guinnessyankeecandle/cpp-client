...
export X25519KeyPair x25519Generate() {
    ...
    sslAssert(EVP_PKEY_keygen(ctx.get(), &raw), "X25519 keygen");
...
export std::vector<uint8_t> x25519DH(const std::vector<uint8_t>& privKey,
    ...
    if (!priv) throw std::runtime_error("X25519 new_raw_private_key failed");

    auto peer = PkeyPtr(
        EVP_PKEY_new_raw_public_key(EVP_PKEY_X25519, nullptr,
                                    peerPub.data(), peerPub.size()));
    if (!peer) throw std::runtime_error("X25519 new_raw_public_key failed");
    ...
    sslAssert(EVP_PKEY_derive_set_peer(ctx.get(), peer.get()), "X25519 derive_set_peer");

    std::size_t sharedLen = X25519_KEY_BYTES;
    std::vector<uint8_t> shared(sharedLen);
    sslAssert(EVP_PKEY_derive(ctx.get(), shared.data(), &sharedLen), "X25519 derive");
    return shared;
}

export std::vector<uint8_t> x25519PublicFromPrivate(const std::vector<uint8_t>& priv) {
    using PkeyPtr = OsslHandle<EVP_PKEY, EVP_PKEY_free>;
    auto pkey = PkeyPtr(
        EVP_PKEY_new_raw_private_key(EVP_PKEY_X25519, nullptr,
                                     priv.data(), priv.size()));
    if (!pkey) throw std::runtime_error("X25519 new_raw_private_key failed");
