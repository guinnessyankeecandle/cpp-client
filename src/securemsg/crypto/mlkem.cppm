...
export MlKemKeyPair mlkemGenerate() {
    ...
    auto ctx = PkeyCtxPtr(EVP_PKEY_CTX_new_from_name(nullptr, "ML-KEM-1024", nullptr));
    if (!ctx) throw std::runtime_error("ML-KEM-1024 CTX_new failed");
...
export MlKemEncapResult mlkemEncap(const std::vector<uint8_t>& pubKey) {
    ...
    auto pub = PkeyPtr(
        EVP_PKEY_new_raw_public_key_ex(nullptr, "ML-KEM-1024", nullptr,
                                       pubKey.data(), pubKey.size()));
    if (!pub) throw std::runtime_error("ML-KEM-1024 new_raw_public_key failed");

    auto ctx = PkeyCtxPtr(EVP_PKEY_CTX_new_from_pkey(nullptr, pub.get(), nullptr));
    if (!ctx) throw std::runtime_error("ML-KEM-1024 CTX_new_from_pkey failed");
    sslAssert(EVP_PKEY_encapsulate_init(ctx.get(), nullptr), "ML-KEM-1024 encap_init");
...
export std::vector<uint8_t> mlkemDecap(const std::vector<uint8_t>& privKey,
    ...
    auto priv = PkeyPtr(
        EVP_PKEY_new_raw_private_key_ex(nullptr, "ML-KEM-1024", nullptr,
                                        privKey.data(), privKey.size()));
    if (!priv) throw std::runtime_error("ML-KEM-1024 new_raw_private_key failed");

    auto ctx = PkeyCtxPtr(EVP_PKEY_CTX_new_from_pkey(nullptr, priv.get(), nullptr));
    if (!ctx) throw std::runtime_error("ML-KEM-1024 CTX_new_from_pkey failed");
    sslAssert(EVP_PKEY_decapsulate_init(ctx.get(), nullptr), "ML-KEM-1024 decap_init");

    std::size_t ssLen = 0;
    sslAssert(EVP_PKEY_decapsulate(ctx.get(), nullptr, &ssLen,
