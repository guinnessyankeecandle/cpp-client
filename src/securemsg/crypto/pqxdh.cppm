...
export struct RemoteKeyBundle {
    std::vector<uint8_t>              ikEdPub;  // Ed25519 pub — used for SPK/PQ signature verification
...
export PqxdhSenderResult pqxdhSend(const X25519KeyPair&   senderIk,
    ...
    auto dh2 = x25519DH(ek.priv,       remote.ikXPub);   // sender EK × receiver IK (X25519)
...
export std::vector<uint8_t> pqxdhReceive(
        const X25519KeyPair&                receiverIk,
        const X25519KeyPair&                receiverSpk,
        const std::optional<X25519KeyPair>& receiverOpk,
        const MlKemKeyPair&                 receiverPq,
        const PqxdhInitialHeader&           header) {

    auto dh1  = x25519DH(receiverSpk.priv, header.senderIkXPub);  // receiver SPK × sender IK
    auto dh2  = x25519DH(receiverIk.priv,  header.ephemeralPub);  // receiver IK × sender EK
