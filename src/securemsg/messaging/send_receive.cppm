...
export SendResult sendDirectMessage(
        ApiClient&                                    api,
        RatchetMap&                                   ratchets,
    ...
    auto result = api.sendMessage(accessToken, recipientId,
...
export void receiveDirectMessages(
    ...
    for (const auto& m : messages) {
        ...
        (void)myUserId; (void)myOpk; (void)myPq; (void)apiForLookup;

        if (!ratchets.contains(senderId)) continue;  // skip if no session yet
        ...
        try {
            ...
            Message msg{id, senderId, myUserId, m.value("ciphertext",""),
                        m.value("ratchet_header_enc",""), now,
...
public:
    ...
    int32_t resolve(int32_t groupId, int32_t incomingEpoch) const {
        ...
        if (it == m_postedEpoch.end()) return incomingEpoch;
        int32_t myPosted = it->second;
        if (incomingEpoch > myPosted)  return incomingEpoch;
        if (incomingEpoch == myPosted) return myPosted;
        return -1;
    }
